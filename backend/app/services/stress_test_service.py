...
class StressTestService:
    ...
    def _load_model(self):
        """Load the PPO model from extracted weights."""
        try:
            from stable_baselines3 import PPO
            # PPO.load expects a .zip or a directory with the right structure
            zip_path = os.path.join(self.model_path, "LiminalAgent.zip")
            if os.path.exists(zip_path):
                self.model = PPO.load(zip_path)
                logger.info("PPO model loaded successfully from zip")
